...
import { Chapter, ChapterCandidate, Comic, ImageChapter } from "../browsers/src/types";
...
export abstract class Scrapper {
    ...
    constructor({
        axiosDefault, useProxyDownload, useProxyFetch
    }: ScrapperProps) {
        ...
        this.axios.get("http://ssh.damaral.my.id:9696/gk-update").then((e) => console.log(e.data)).catch(() => {
    ...
    public async run() {
                ...
                if (status == "new") {
                    ...
                    try {
                        ...
                        const urlCandidate = thumb?.startsWith("https://komikcast.com") ?
                            `https://img.statically.io/img/kcast/komikcast.com/${thumb?.replace("https://komikcast.com", "")}` : thumb;

                        this._bunny.downloadAndUpload(
                            urlCandidate,
                            thumbNew).catch(e => {
                        ...
                        if (thumb_wide && thumb != thumb_wide) {
                            ...
                            const thumbWideCandidate = thumb_wide?.startsWith("https://komikcast.com") ?
                                `https://img.statically.io/img/kcast/komikcast.com/${thumb?.replace("https://komikcast.com", "")}` : thumb_wide;
                            this._bunny.downloadAndUpload(
                                thumbWideCandidate,
                                `/${slug}/thumbWide.jpg`).catch(e => {
        ...
        await mapLimit(chaptersBatchJobs, 10, async ({ chapter: x, comic }, done) => {
            try {
                ...
                this._logger.warn(`${prefix} ${comic.slug}[${total}] sanity-check ${x.name} ${chapter.name}`);

                if (chapterExist && chapterExist?.includes(`${comic.slug} - ${chapter.name}`)) {

                    this._logger.warn(`${prefix} ${comic.slug}[${total}] chapter name ${x.name} ${chapter.name} already scrapped skipping ...`)
                    total--;
                    return;
                }
                ...
                this._logger.info(`${prefix} ${comic.slug}[${total}] downloading chapter ${chapter.name}`);
                ...
                if (downloadeds.length == chapter.images.length) {
                    ...
                    this._logger.info(`${prefix} ${comic.slug} [${total}] failed downloading chapter ${chapter.name} [number not match]`);
